...
def temp_jj_repo():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Initialize jj repo
        code, out, err = run_jj(["jj", "init", "--git"], repo_path)
        assert code == 0, f"Failed to init jj repo: {err}"
        ...
        run_jj(["jj", "commit", "-m", "initial commit"], repo_path)
...
""")
        ...
        # Run the script
        code, out, err = run_jj(
            [sys.executable, str(dot_to_jj_script), str(dot_file),
...
class TestDotToJjDagErrorHandling:
    ...
    def test_missing_dot_file(self, temp_jj_repo, dot_to_jj_script):
        ...
        code, out, err = run_jj(
            [sys.executable, str(dot_to_jj_script),
